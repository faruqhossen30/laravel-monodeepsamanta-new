...
@prepend('script')
    ...
    <script>
        ...
        $(document).on('click', 'input[name="photo"]', function(e) {
            ...
            $(`#dropZoneArea${thumbnailNumber}`).empty();
            $(`#dropZoneArea${thumbnailNumber}`).append(
                `
                <div class="relative group">
                    <button type="button" id="removeThumbnail" onclick="removeThubnailFunction(${thumbnailNumber})"
                        class="absolute removeThumbnail right-0 p-2 m-4 hidden group-hover:block transition duration-100">
                    ...
                    <input type="hidden" name="thumbnail[${thumbnailNumber}]" value="${$(this).val()}">
        ...
        function removeThubnailFunction(itemNumber) {
            // $(document).on('click', '.removeThumbnail', function(e) {
                console.log('removethumbnail');

                $(`#dropZoneArea${itemNumber}`).empty();
                $(`#dropZoneArea${itemNumber}`).append(
                    `
